...
class SettingsConnectionMatcher(
    ...
    fun getProjectWithMatchingConnection(settingsJson: String): String? {
        try {
            ...
            val jsonProtocol = try {
                jsonSettings.get(ProjectKeys.KEY_PROTOCOL)
            } catch (e: JSONException) {
                Defaults.unprotected[ProjectKeys.KEY_PROTOCOL]!!
            }

            val jsonUrl = try {
                jsonSettings.get(ProjectKeys.KEY_SERVER_URL)
            } catch (e: JSONException) {
                Defaults.unprotected[ProjectKeys.KEY_SERVER_URL]!!
            }
            val jsonUsername = try {
                jsonSettings.get(ProjectKeys.KEY_USERNAME)
            } catch (e: JSONException) {
                ""
            }

            val jsonGoogleAccount = try {
                jsonSettings.get(ProjectKeys.KEY_SELECTED_GOOGLE_ACCOUNT)
            } catch (e: JSONException) {
                ""
            }
            ...
            projectsRepository.getAll().forEach {
                ...
                val projectGoogleAccount =
                    projectSettings.getString(ProjectKeys.KEY_SELECTED_GOOGLE_ACCOUNT)
